...
class WebhookSender:
    ...
    def _send_webhook(self, endpoint: str, payload: Dict[str, Any], session_id: str = "N/A"):
        ...
        if not self.discord_api_base_url:
            ...
            logger.error("'discord_api_base_url' is not defined in WeeChat config. Cannot send webhook.")
            return
        ...
        # --- MODIFIED: Changed to logger.debug for less verbose default logging ---
        logger.debug(f"Sending webhook for session {session_id} to {api_url} with payload: {json_payload[:100]}...")
