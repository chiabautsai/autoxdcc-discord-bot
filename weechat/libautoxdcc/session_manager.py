...
# --- MODIFIED: Import the logger instance directly ---
from . import utils
...
class SessionManager:
    ...
    def start_new_session(self, session_id: str, query: str, session_type: str):
        ...
        # --- MODIFIED: Changed to logger.info ---
        logger.info(f"Starting new session. Type: '{session_type}', Query: '{query}', SID: {session_id}")

        server_buffer_ptr = weechat.info_get("irc_buffer", self.irc_server_name)
        if not server_buffer_ptr:
            # --- MODIFIED: Changed to logger.error ---
            logger.error(f"Could not find server buffer for '{self.irc_server_name}'.")
        ...
        if not channel_buffer_ptr:
            # --- MODIFIED: Changed to logger.error ---
            logger.error(f"Could not find channel buffer '{full_channel_name}'.")
            self.end_session(session_id, release_lock=True)
    ...
    def end_session(self, session_id: str, release_lock: bool = False):
        ...
        if session_id in self._sessions:
            # --- MODIFIED: Changed to logger.debug ---
            logger.debug(f"Cleaning up data for session {session_id}.")
    ...
    def release_search_lock(self):
        ...
        if self.is_search_active:
            ...
            # --- MODIFIED: Changed to logger.info ---
            logger.info("Search lock released.")
    ...
    def handle_print_callback(self, session_id: str, message: str) -> int:
        ...
        if session.type == 'search':
            ...
            if is_end:
                # --- MODIFIED: Changed to logger.debug ---
                logger.debug(f"End of search results detected for session {session_id}.")
            ...
            if header_summary:
                ...
                # --- MODIFIED: Changed to logger.debug ---
                logger.debug(f"HOT PARSE: Matched header. Summary: '{session.hot_summary}'")
                line_matched = True
            if hot_item_dict:
                session.add_hot_item(hot_item_dict)
                # --- MODIFIED: Changed to logger.debug ---
                logger.debug(f"HOT PARSE: Matched item. Grabs='{hot_item_dict['grabs']}', Cat='{hot_item_dict['category']}', File='{hot_item_dict['filename']}'")
                line_matched = True
    ...
    def handle_final_processing(self, session_id: str) -> int:
        ...
        session = self._sessions.get(session_id)
        if not session:
            # --- MODIFIED: Changed to logger.warning ---
            logger.warning(f"handle_final_processing called for ended session {session_id}.")
            self.release_search_lock()
            return weechat.WEECHAT_RC_OK
    ...
    def handle_expiry(self, session_id: str) -> int:
        ...
        if session_id in self._sessions:
            # --- MODIFIED: Changed to logger.info ---
            logger.info(f"Session '{session_id}' has expired and will be terminated.")
    ...
    def handle_http_post_callback(self, session_id: str, command: str, return_code: int, stdout: str, stderr: str) -> int:
        ...
        if return_code != 0:
            # --- MODIFIED: Changed to logger.error ---
            logger.error(f"Failed to send webhook for session {session_id} (RC: {return_code}). Curl stderr: {stderr.strip()}")
        else:
            # --- MODIFIED: Changed to logger.debug ---
            logger.debug(f"Webhook for session {session_id} sent successfully.")
        return weechat.WEECHAT_RC_OK
    ...
    def shutdown(self):
        ...
        # --- MODIFIED: Changed to logger.info ---
        logger.info(f"Shutting down. Terminating {len(self._sessions)} active session(s)...")
...
def service_search_cb(data, buffer, args, session_manager_instance: SessionManager) -> int:
    ...
    try:
        ...
        # --- MODIFIED: Changed to logger.error ---
        logger.error("Usage: /autoxdcc_service_search <session_id> <query>")
        return weechat.WEECHAT_RC_ERROR
    
    if session_manager_instance.is_search_active:
        # --- MODIFIED: Changed to logger.info ---
        logger.info(f"Rejecting search for '{query}' (SID: {session_id}). A search is already active.")
    ...
    # --- MODIFIED: Changed to logger.info ---
    logger.info(f"Search lock acquired for search session {session_id}.")
...
def service_hot_cb(data, buffer, args, session_manager_instance: SessionManager) -> int:
    ...
    try:
        ...
        # --- MODIFIED: Changed to logger.error ---
        logger.error("Usage: /autoxdcc_service_hot <session_id>")
        return weechat.WEECHAT_RC_ERROR
    
    if session_manager_instance.is_search_active:
        # --- MODIFIED: Changed to logger.info ---
        logger.info(f"Rejecting hot list request (SID: {session_id}). A search is already active.")
        session_manager_instance.webhook_sender.send_rejection(session_id, "Another search is already in progress. Please try again.")
        return weechat.WEECHAT_RC_OK

    session_manager_instance.is_search_active = True
    # --- MODIFIED: Changed to logger.info ---
    logger.info(f"Search lock acquired for hot list session {session_id}.")
...
def service_download_cb(data, buffer, args, session_manager_instance: SessionManager) -> int:
    ...
    try:
        ...
        # --- MODIFIED: Changed to logger.error ---
        logger.error("Usage: /autoxdcc_service_download <session_id> <choice_id>");
        return weechat.WEECHAT_RC_ERROR
    ...
    if not session or session.type != 'search':
        ...
        # --- MODIFIED: Changed to logger.warning ---
        logger.warning(f"Session '{session_id}' is invalid for download request or expired.")
    ...
    if command_to_run and target_filename:
        ...
        if channel_buffer_ptr:
            # --- MODIFIED: Changed to logger.info ---
            logger.info(f"Executing download for session {session_id}, choice {choice_id_str}: {command_to_run}")
            ...
            session_manager_instance.end_session(session_id)
        else:
